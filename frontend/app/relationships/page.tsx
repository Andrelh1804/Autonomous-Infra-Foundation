    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-card border border-border rounded-2xl w-full max-w-md mx-4 shadow-2xl">
        ...
        <div className="p-6 space-y-4">
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-1">Source Asset</label>
            <select value={sourceId} onChange={e => setSourceId(e.target.value)}
              ...
              {assets.map(a => <option key={a.id} value={a.id}>{a.hostname || a.ip_address || `Asset #${a.id}`}</option>)}
            </select>
          </div>
          <div>
          ...
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-1">Target Asset</label>
            <select value={targetId} onChange={e => setTargetId(e.target.value)}
              className="w-full bg-background border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
              <option value="">— Select target —</option>
              {assets.filter(a => String(a.id) !== sourceId).map(a => <option key={a.id} value={a.id}>{a.hostname || a.ip_address || `Asset #${a.id}`}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-1">Description <span className="font-normal">(optional)</span></label>
          ...
          {error && (
            <div className="flex items-center gap-2 text-red-400 text-sm bg-red-400/10 rounded-lg px-3 py-2">
              ...
              {(error as any)?.response?.data?.detail || 'Failed to add relationship'}
            </div>
          )}
    ...
    <Layout>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold">Relationship Engine</h1>
            <p className="text-muted-foreground text-sm mt-1">
              Map dependencies and connections between assets
            </p>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="bg-card border border-border rounded-xl p-12 text-center">
            ...
            <p className="text-muted-foreground">No relationships defined yet. Add connections between assets.</p>
          </div>
        ) : (
              ...
              return (
                <div key={type} className="bg-card border border-border rounded-xl overflow-hidden">
                  <div className="px-5 py-3 border-b border-border flex items-center gap-3">
                    ...
                    <span className="text-xs text-muted-foreground">{group.length} relationship{group.length !== 1 ? 's' : ''}</span>
                  </div>
                  <div className="divide-y divide-border">
                    {group.map(rel => (
                      <div key={rel.id} className="px-5 py-3 flex items-center gap-3">
                        ...
                        <button onClick={() => { if (confirm('Remove relationship?')) deleteMutation.mutate(rel.id); }}
