  ...
  return (
    <Layout>
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold">Metrics Viewer</h1>
          <p className="text-muted-foreground text-sm mt-1">Time-series performance data for monitored devices</p>
        </div>
        ...
        <div className="flex gap-3 flex-wrap items-center">
          <div className="flex items-center gap-2 bg-card border border-border rounded-xl px-3 py-2 min-w-[240px]">
            ...
            <select value={activeTarget ?? ''} onChange={e => setSelectedTarget(Number(e.target.value))} className="bg-transparent text-sm flex-1 outline-none">
          ...
          <div className="flex gap-1 bg-muted/30 rounded-xl p-1">
            {[6, 12, 24, 48, 168].map(h => (
              <button key={h} onClick={() => setHours(h)} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${hours === h ? 'bg-card shadow text-foreground' : 'text-muted-foreground hover:text-foreground'}`}>
        ...
        {!activeTarget ? (
          <div className="py-20 text-center bg-card border border-border rounded-xl">
            ...
            <p className="text-muted-foreground">Select a target to view metrics</p>
          </div>
        ) : isLoading ? (
          ...
          <div className="py-20 text-center bg-card border border-border rounded-xl">
            ...
            <p className="text-xs text-muted-foreground mt-1">Poll the target from the Monitoring page to collect data</p>
          </div>
        ) : (
          <div className="grid sm:grid-cols-2 gap-4">
            {metricKeys.map(mKey => {
                ...
                <div key={mKey} className="bg-card border border-border rounded-xl p-5 overflow-hidden">
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">{samples.length} samples · {hours}h window</p>
                    </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 text-center">
                    {[['Avg', avg.toFixed(1)], ['Max', max.toFixed(1)], ['Min', (values.length ? Math.min(...values) : 0).toFixed(1)]].map(([k, v]) => (
