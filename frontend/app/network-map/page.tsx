...
const TYPE_NAMES: Record<string, string> = {
  server: 'Servidor', workstation: 'Estação de Trabalho', switch: 'Switch',
  router: 'Roteador', firewall: 'Firewall', access_point: 'Access Point',
  storage: 'Storage', virtual_machine: 'Máquina Virtual', cloud_resource: 'Recurso Cloud',
  printer: 'Impressora', database: 'Banco de Dados', unknown: 'Desconhecido',
};
  ...
  return (
    <div className={`relative border rounded-xl p-3 w-44 cursor-default transition-all hover:scale-105 hover:shadow-lg ${color}`}>
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-xs font-semibold truncate">{asset.asset_type?.name ?? 'Ativo'}</span>
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Visão visual de {assetsData?.total ?? 0} ativos na sua infraestrutura
          </p>
        </div>
        ...
        <div className="bg-card border border-border rounded-xl overflow-hidden">
          <div className="px-5 py-4 border-b border-border flex items-center gap-3">
            ...
            <h2 className="font-semibold">Mapa de Infraestrutura</h2>
            <span className="ml-auto text-xs text-muted-foreground">{sortedGroups.length} grupo{sortedGroups.length !== 1 ? 's' : ''}</span>
          </div>

          {isLoading ? (
            <div className="p-12 text-center">
              <div className="w-8 h-8 border-2 border-indigo-500 border-t-transparent rounded-full animate-spin mx-auto" />
              <p className="text-muted-foreground text-sm mt-3">Carregando mapa…</p>
            </div>
          ) : assets.length === 0 ? (
            <div className="p-12 text-center">
              <Network className="w-12 h-12 text-muted-foreground mx-auto mb-3 opacity-30" />
              <p className="text-muted-foreground">Nenhum ativo encontrado. Execute uma varredura de discovery para popular seu mapa.</p>
            </div>
          ) : (
            <div className="p-6 space-y-8">
              {sortedGroups.map(typeSlug => {
                ...
                const typeName = TYPE_NAMES[typeSlug] || group[0]?.asset_type?.name || typeSlug.replace('_', ' ');
                return (
                  <div key={typeSlug}>
        ...
        <div className="bg-card border border-border rounded-xl p-4">
          ...
          <div className="flex flex-wrap gap-3">
            <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
              <div className="w-2.5 h-2.5 rounded-full bg-emerald-400" /> Ativo
            </div>
            <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
              <div className="w-2.5 h-2.5 rounded-full bg-slate-500" /> Inativo / Desconhecido
            </div>
            {Object.entries(TYPE_COLORS).slice(0, 6).map(([slug, color]) => {
              ...
              return (
                <div key={slug} className={`flex items-center gap-1.5 text-xs px-2 py-0.5 rounded-md border ${color}`}>
                  ...
                  {TYPE_NAMES[slug] || slug.replace('_', ' ')}
                </div>
              );
            })}
          </div>
