      ...
      <div className="flex-1 min-w-0">
        ...
        {supply.days_remaining != null && (
          <p className="text-[10px] text-muted-foreground mt-0.5">
            {supply.days_remaining < 1 ? 'Acabando em breve' : `~${Math.round(supply.days_remaining)}d restante${Math.round(supply.days_remaining) !== 1 ? 's' : ''}`}
          </p>
        )}
      </div>
    </div>
  ...
  return (
    <Layout>
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold">Monitoramento de Impressoras</h1>
          <p className="text-muted-foreground text-sm mt-1">Níveis de suprimento, contadores de páginas e alertas preditivos</p>
        </div>
          ...
          <div className="bg-red-500/8 border border-red-500/20 rounded-xl p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm font-semibold text-red-400">
                {critItems.length} alerta{critItems.length > 1 ? 's' : ''} crítico{critItems.length > 1 ? 's' : ''} de suprimento
              </span>
            </div>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2">
              {critItems.slice(0, 6).map((s: any) => (
                <div key={s.id} className="bg-card/50 border border-border rounded-lg px-3 py-2">
                  ...
                  <p className={`text-xs font-semibold ${RISK_COLORS[s.risk_level]}`}>
                    {s.level_percent?.toFixed(0)}%{s.days_remaining != null ? ` · ~${Math.round(s.days_remaining)}d restante${Math.round(s.days_remaining) !== 1 ? 's' : ''}` : ''}
                  </p>
                </div>
              ))}
            </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <div className="bg-card border border-border rounded-xl p-4 flex items-center gap-3">
            ...
            <div><p className="text-xs text-muted-foreground">Suprimento Baixo</p><p className="text-xl font-bold">{critItems.length}</p></div>
          </div>
          <div className="bg-card border border-border rounded-xl p-4 flex items-center gap-3">
        ...
        <div className="relative max-w-sm">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar impressoras…"
        ...
        {isLoading ? (
          ...
          <div className="py-16 text-center bg-card border border-border rounded-xl">
            ...
            <p className="text-muted-foreground">Nenhuma impressora monitorada</p>
            <p className="text-xs text-muted-foreground mt-1">Adicione um alvo com o tipo de dispositivo "impressora"</p>
          </div>
        ) : (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {printers.map((p: any) => (
              <div key={p.id} className="bg-card border border-border rounded-xl p-5">
                ...
                {p.supplies && p.supplies.length > 0 ? (
                  ...
                  <div className="py-4 text-center text-xs text-muted-foreground">Sem dados de suprimento — sonde para coletar</div>
                )}
                <div className="mt-3 pt-3 border-t border-border flex items-center justify-between">
                  <span className="text-xs text-muted-foreground">Última sondagem: {relTime(p.last_polled_at)}</span>
