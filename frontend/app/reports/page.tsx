...
export default function ReportsPage() {
  ...
  function downloadCSV() {
    const rows = [
      ['Dispositivo', 'Host', 'Tipo', 'Online', 'Pontuação de Saúde', 'Última Sondagem'],
      ...(targets?.items ?? []).map((t: any) => [
        t.name, t.host, t.device_type,
        t.is_online === true ? 'Sim' : t.is_online === false ? 'Não' : 'Desconhecido',
        t.health_score ?? 100,
        t.last_polled_at ?? 'Nunca',
      ]),
    ];
    const csv = rows.map(r => r.join(',')).join('\n');
    const a = document.createElement('a');
    a.href = `data:text/csv;charset=utf-8,${encodeURIComponent(csv)}`;
    a.download = `nexaops-relatorio-monitoramento-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
  }
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">Resumo de saúde da infraestrutura e dados exportáveis</p>
          </div>
        ...
        <div className="bg-card border border-border rounded-xl p-6">
          <h2 className="font-semibold mb-5 flex items-center gap-2"><TrendingUp className="w-4 h-4 text-indigo-400" /> Visão Geral de Saúde da Plataforma</h2>
          <div className="flex flex-wrap gap-8 justify-around">
            <Gauge value={uptime}      label="Disponibilidade %"     color="#10b981" />
            <Gauge value={health}      label="Pontuação de Saúde Média" color="#6366f1" />
            <Gauge value={resolveRate} label="Taxa de Resolução %"    color="#f59e0b" />
          </div>
        </div>

        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { icon: Activity,      label: 'Total Monitorado',   value: monSum?.total ?? 0,        sub: `${monSum?.online ?? 0} online`,     color: 'bg-indigo-600' },
            { icon: AlertTriangle, label: 'Eventos Abertos',    value: evStats?.open ?? 0,        sub: `${evStats?.critical ?? 0} críticos`, color: 'bg-amber-600' },
            { icon: Printer,       label: 'Alertas de Impressora', value: (critical ?? []).length, sub: 'suprimento baixo/vazio',           color: 'bg-red-600' },
            { icon: CheckCircle2,  label: 'Eventos Resolvidos', value: evStats?.resolved ?? 0,    sub: 'total resolvido',                   color: 'bg-emerald-600' },
        ...
        {monSum?.by_device_type && Object.keys(monSum.by_device_type).length > 0 && (
          <div className="bg-card border border-border rounded-xl p-5">
            <h2 className="font-semibold mb-4 flex items-center gap-2"><Activity className="w-4 h-4 text-indigo-400" /> Dispositivos por Tipo</h2>
        ...
        {(critical ?? []).length > 0 && (
          <div className="bg-card border border-border rounded-xl overflow-hidden">
            <div className="px-5 py-3 border-b border-border flex items-center gap-2">
              ...
              <h2 className="font-semibold text-sm">Suprimentos Críticos de Impressora</h2>
            </div>
              ...
              {(critical ?? []).slice(0, 10).map((s: any) => (
                <div key={s.id} className="px-5 py-3 flex items-center justify-between gap-4">
                  ...
                  <div className="text-right">
                    ...
                    {s.days_remaining != null && <p className="text-xs text-muted-foreground">~{Math.round(s.days_remaining)}d restante{Math.round(s.days_remaining) !== 1 ? 's' : ''}</p>}
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="bg-card border border-border rounded-xl overflow-hidden">
          <div className="px-5 py-3 border-b border-border flex items-center justify-between">
            <h2 className="font-semibold text-sm flex items-center gap-2"><FileText className="w-4 h-4 text-indigo-400" /> Alvos de Monitoramento</h2>
            <span className="text-xs text-muted-foreground">{targets?.total ?? 0} total</span>
          </div>
          <div className="divide-y divide-border">
            {recentTargets.length === 0 ? (
              <div className="py-8 text-center text-muted-foreground text-sm">Nenhum alvo de monitoramento</div>
              ...
              <div key={t.id} className="px-5 py-3 flex items-center gap-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {t.is_online === null  && <span className="text-xs text-muted-foreground bg-muted/30 px-2 py-0.5 rounded-full">Desconhecido</span>}
                  <span className="text-xs text-muted-foreground w-20 text-right">{t.last_polled_at ? relTime(t.last_polled_at) : 'Nunca'}</span>
                </div>
              </div>
            ))}
          </div>
