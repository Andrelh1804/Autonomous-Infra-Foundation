  ...
  return (
    <Layout>
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold">Events & Incidents</h1>
          <p className="text-muted-foreground text-sm mt-1">Monitor and respond to infrastructure alerts</p>
        </div>

        {/* Stats */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            { label: 'Open', value: stats?.open ?? 0, color: 'bg-amber-600' },
            { label: 'Critical', value: stats?.critical ?? 0, color: 'bg-red-600' },
            { label: 'Acknowledged', value: stats?.acknowledged ?? 0, color: 'bg-blue-600' },
            { label: 'Resolved (total)', value: stats?.resolved ?? 0, color: 'bg-emerald-600' },
        ...
        <div className="flex gap-1 bg-muted/30 rounded-xl p-1 w-fit">
          {['events', 'incidents'].map(t => (
            <button key={t} onClick={() => setTab(t as any)} className={cn('px-4 py-1.5 rounded-lg text-sm font-medium transition capitalize', tab === t ? 'bg-card shadow text-foreground' : 'text-muted-foreground hover:text-foreground')}>
              {t}
            </button>
          ))}
        </div>
        ...
        <div className="flex gap-3 flex-wrap">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search…" className="input-base w-full pl-9" />
          </div>
          <select value={severity} onChange={e => setSeverity(e.target.value)} className="input-base">
            <option value="">All severities</option>
            {['info','warning','critical','emergency'].map(s => <option key={s} value={s}>{s}</option>)}
          </select>
          ...
          <div className="bg-card border border-border rounded-xl overflow-hidden">
            {isLoading ? (
              ...
              <div key={e.id} className="px-5 py-3 border-b border-border last:border-0 flex items-center gap-4 hover:bg-muted/20 transition">
                <span className={`text-xs px-2 py-0.5 rounded-full border font-medium flex-shrink-0 ${SEV_COLOR[e.severity] ?? SEV_COLOR.info}`}>{e.severity}</span>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium truncate">{e.title}</p>
                  <p className="text-xs text-muted-foreground">{e.target_name ? `${e.target_name} · ` : ''}{relTime(e.created_at)}</p>
                </div>
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium flex-shrink-0 ${STATUS_COLOR[e.status] ?? ''}`}>{e.status}</span>
                <div className="flex gap-1 flex-shrink-0">
                  {e.status === 'open' && (
                    <button onClick={() => ackMut.mutate(e.id)} className="text-xs px-2 py-1 rounded bg-blue-600/15 hover:bg-blue-600/25 text-blue-400 transition">Ack</button>
                  )}
                  {e.status !== 'resolved' && (
                    <button onClick={() => resMut.mutate(e.id)} className="text-xs px-2 py-1 rounded bg-emerald-600/15 hover:bg-emerald-600/25 text-emerald-400 transition">Resolve</button>
                  )}
                </div>
              </div>
            ))}
              ...
              <div key={inc.id} className="px-5 py-4 border-b border-border last:border-0 flex items-start gap-4 hover:bg-muted/20 transition">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className={`text-xs px-2 py-0.5 rounded-full border font-medium ${SEV_COLOR[inc.severity] ?? SEV_COLOR.info}`}>{inc.severity}</span>
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${STATUS_COLOR[inc.status] ?? ''}`}>{inc.status}</span>
                  </div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">{relTime(inc.created_at)} · {inc.event_count} event{inc.event_count !== 1 ? 's' : ''}</p>
                </div>
                <div className="flex gap-1 flex-shrink-0">
                  {inc.status === 'open' && (
                    <button onClick={() => ackIncMut.mutate(inc.id)} className="text-xs px-2 py-1 rounded bg-blue-600/15 hover:bg-blue-600/25 text-blue-400 transition">Ack</button>
                  )}
                  {inc.status !== 'resolved' && (
                    <button onClick={() => resIncMut.mutate(inc.id)} className="text-xs px-2 py-1 rounded bg-emerald-600/15 hover:bg-emerald-600/25 text-emerald-400 transition">Resolve</button>
                  )}
                </div>
              </div>
            ))}
